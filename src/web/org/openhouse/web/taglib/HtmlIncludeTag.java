...
public class HtmlIncludeTag extends TagSupport {
	...
	public static final String openhouse_HTML_INCLUDE_REQUEST_ID_KEY = "org.openhouse.htmlInclude.pageName";
	
	public static final String openhouse_HTML_INCLUDE_MAP_KEY = "org.openhouse.htmlInclude.includeMap";
	...
	public int doStartTag() throws JspException {
		...
		if (isJs || isCss) {
			...
			log.debug("Object at pageContext." + HtmlIncludeTag.openhouse_HTML_INCLUDE_MAP_KEY + " is "
			        + pageContext.getAttribute(HtmlIncludeTag.openhouse_HTML_INCLUDE_MAP_KEY, PageContext.SESSION_SCOPE) + "");
			...
			if (!isAlreadyUsed(file, initialRequestId)) {
				...
				// the openhouse version is inserted into the file src so that js and css files are cached across version releases
				if (isJs) {
					output = "<script src=\"" + prefix + file + "?v=" + OpenhouseConstants.OPENHOUSE_VERSION_SHORT + "\" type=\"text/javascript\" ></script>";
				} else if (isCss) {
					output = "<link href=\"" + prefix + file + "?v=" + OpenhouseConstants.OPENHOUSE_VERSION_SHORT + "\" type=\"text/css\" rel=\"stylesheet\" />";
				}
	...
	private boolean isAlreadyUsed(String fileName, String initialRequestId) {
		...
		if (fileName != null) {
			...
			String lastRequestId = (String) pageContext.getAttribute(HtmlIncludeTag.openhouse_HTML_INCLUDE_REQUEST_ID_KEY,
			    PageContext.SESSION_SCOPE);
			
			// retrieve the htmlinclude map from the page request
			//HashMap<String,String> hmIncludeMap = (HashMap<String, String>) initialRequest.getAttribute(HtmlIncludeTag.openhouse_HTML_INCLUDE_KEY);
			HashMap<String, String> hmIncludeMap = (HashMap<String, String>) pageContext.getAttribute(
			    HtmlIncludeTag.openhouse_HTML_INCLUDE_MAP_KEY, PageContext.SESSION_SCOPE);
			...
			if (hmIncludeMap.containsKey(fileName)) {
				...
				pageContext.setAttribute(HtmlIncludeTag.openhouse_HTML_INCLUDE_MAP_KEY, hmIncludeMap,
				    PageContext.SESSION_SCOPE);
				
				// save the name of the initial page 
				pageContext.setAttribute(HtmlIncludeTag.openhouse_HTML_INCLUDE_REQUEST_ID_KEY, initialRequestId,
				    PageContext.SESSION_SCOPE);
			}
		}
